...
- 代码生成与执行：生成并执行 JavaScript 和 Python 代码
- 数据可视化：从结构化数据生成可视化图表
- 图片生成与编辑：利用 AI 生成和编辑图片（需要OpenAI的API Key）
- 聊天归档与管理：支持可配置的保留策略
- 实时语音聊天：基于 OpenAI `gpt-realtime`（需要OpenAI的API Key）
- 网络搜索：基于Exa API的增强的搜索能力（需要Exa的API Key）
...
# 2. 使用 Docker Compose 构建并启动所有服务（包含 PostgreSQL 和 RustFS）
